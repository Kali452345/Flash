 ...
 *
 * Pure and synchronized; no coroutines, no Android types, no repository reference. The caller polls it
 * on the session-up edge and issues the returned ids to `resumeTransfer`.
 */
public class TransferReconnectResumePolicy(
    ...
    public fun retainOnly(liveIds: Set<FlashTransferId>) {
        val live = liveIds.mapTo(HashSet(liveIds.size)) { id -> id.value }
    ...
    public fun attemptsFor(transferId: FlashTransferId): Int =
        attempts[transferId.value]?.count ?: 0
