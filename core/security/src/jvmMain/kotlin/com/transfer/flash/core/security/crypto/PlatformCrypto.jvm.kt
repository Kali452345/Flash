...
/**
 * Desktop-JVM `actual`s for the [PlatformCrypto] seam — a direct transcription of the JCA calls that
 * `core/security`'s shared code made before Phase 07, so behaviour is bit-identical (R8).
 *
 * This file is byte-for-byte the same as its `androidMain` counterpart. That duplication is
