...
internal object TofuPolicy {
    ...
    /**
     * Constant-time hex equality (`MessageDigest.isEqual` on JVM targets) so pin checks don't
     * leak match position through timing. Callers pass values already run through
     * [NumericComparisonCode.normalizeHex], which emits only `[0-9a-f]` — so UTF-8 and US-ASCII
     * encodings of the input are byte-identical.
     */
    private fun constantTimeEquals(aHex: String, bHex: String): Boolean =
        constantTimeBytesEqual(
            aHex.encodeToByteArray(),
            bHex.encodeToByteArray(),
        )
}
