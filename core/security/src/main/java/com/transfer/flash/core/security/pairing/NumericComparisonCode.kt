 ...
 *
 * Pure function: no I/O, no clock, deterministic across JVMs (SHA-256 via
...
object NumericComparisonCode {
    ...
    fun derive(fingerprintHexA: String, fingerprintHexB: String): String {
        ...
        val digest = MessageDigest.getInstance("SHA-256")
            .digest(first.toByteArray(Charsets.US_ASCII) + second.toByteArray(Charsets.US_ASCII))
        var value = 0L
    ...
    fun confirmationHashHex(code: String): String {
        val payload = "flash-pairing-confirm-v1:$code".toByteArray(Charsets.US_ASCII)
    ...
    fun hashesEqual(expectedHex: String, presentedHex: String): Boolean =
        MessageDigest.isEqual(
            normalizeHex(expectedHex).toByteArray(Charsets.US_ASCII),
            normalizeHex(presentedHex).toByteArray(Charsets.US_ASCII),
        )
